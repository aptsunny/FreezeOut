...
def calc_speedup(growthRate, nDenseBlocks, t_0, how_scale):
    ...
    # explain
    # for index, c_i in enumerate(c): # dence layer flops
    #     aab = scale_fn[how_scale]
    #     xxc = (t_0 + (1 - t_0) * float(index)/len(c))
    #     once_com = c_i * aab(xxc) # why depend on 0.8 -> (0.8)^3=0.512
    #     once_com = c_i * scale_fn[how_scale]()
    #     print(once_com)
